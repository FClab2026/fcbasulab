...
import {
  fetchAlumniPageAction,
  type AlumniItem,
} from '@/lib/load_data/loadAlumni'
import { type AlumniCategory } from "@/lib/enums"

interface Props {
  category: AlumniCategory
  displayName: string
  initialItems: AlumniItem[]
  totalItems: number
...
const CategorySection = ({
  ...
  const [items, setItems] = useState<AlumniItem[]>(initialItems)
  ...
  const goToPage = useCallback(
    async (targetPage: number) => {
      ...
      setLoading(true)
      try {
        const result = await fetchAlumniPageAction(
          category,
          targetPage,
          pageSize,
        )
  ...
  return (
    <section
      aria-labelledby={`alumni-${category}`}
          ...
          >
            {items.map((item, index) => (
              <motion.li
                ...
                className="group relative grid grid-cols-[52px_1fr] md:grid-cols-[72px_1fr] gap-3 md:gap-5 py-2.5 border-b border-slate-100 last:border-b-0"
              ...
              >
                ...
                <div className="relative">
                  <span
                    aria-hidden="true"
                    className="absolute left-[calc(100%-1px)] top-0 bottom-[-10px] w-px bg-slate-200 group-last:bottom-1/2"
                  />
                  <span
                    aria-hidden="true"
                    className="absolute -right-[4px] top-1 w-2 h-2 rounded-full bg-white border-2 border-slate-300 group-hover:border-amber-700 group-hover:bg-amber-700 transition-colors"
                  />
                  <time
                    dateTime={String(item.year ?? new Date(item.createdAt).getFullYear())}
                    className="block text-right pr-4 font-serif text-sm text-slate-900 tabular-nums leading-tight"
                  >
                    {item.year ?? new Date(item.createdAt).getFullYear()}
                  </time>
                </div>

                {/* Right: name + HTML body inline, clamped to 2 lines so
                    row height never varies with content length */}
                <div className="min-w-0 flex items-center">
                  <div className="line-clamp-2 leading-snug">
                    <span className="font-semibold text-[13px] text-slate-900">
                      {item.name}
                      {item.body && <span className="text-slate-400">: </span>}
                    </span>
                    <article
                      className="
